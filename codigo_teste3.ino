...
void loop() {
  ...
  if (is_black(valorSensorM) == true) { //se o sensor do meio estiver no preto
    if (is_black(valorSensorE) == false) { //se o sensor esquerdo estiver no branco
      praEsquerda(80, 150); //XX1 é a velocidade do motor da esquerda, que devera ser menor. Testar se deve ou nao ir para tras na robotica
    } else if (is_black(valorSensorD) == false) { //se o sensor direito estiver no branco
      praDireita(150, 80); //XX2 é a velocidade do motor da direita, que devera ser menor. Testar se deve ou nao ir para tras na robotica
    }
  } else {
    if (is_black(valorSensorM) == false && is_black(valorSensorE) == false && is_black(valorSensorD) == false) {
      praFrente(150); //velocidade dos dois motores, porque os dois tem que ir pra frente na mesma velocidade
